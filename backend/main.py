...
from fastapi import FastAPI, HTTPException, Depends, status, WebSocket, WebSocketDisconnect, Query, Request, UploadFile, File, Header
...
LEGAL_TORRENT_AUTODOWNLOAD_ENABLED = os.getenv("LEGAL_TORRENT_AUTODOWNLOAD_ENABLED", "0").strip() == "1"
LEGAL_TORRENT_ALLOWED_HOSTS = {
    host.strip().lower()
    for host in os.getenv("LEGAL_TORRENT_ALLOWED_HOSTS", "").split(",")
    if host.strip()
}
LEGAL_TORRENT_DOWNLOAD_DIR = os.getenv("LEGAL_TORRENT_DOWNLOAD_DIR", "/tmp/qulte-legal-torrents").strip() or "/tmp/qulte-legal-torrents"
LEGAL_TORRENT_RCLONE_DESTINATION = os.getenv("LEGAL_TORRENT_RCLONE_DESTINATION", "").strip()
LEGAL_TORRENT_ADMIN_TOKEN = os.getenv("LEGAL_TORRENT_ADMIN_TOKEN", "").strip()
LEGAL_TORRENT_JOB_TIMEOUT_SECONDS = int(os.getenv("LEGAL_TORRENT_JOB_TIMEOUT_SECONDS", "21600") or "21600")
...
legal_torrent_executor = ThreadPoolExecutor(max_workers=int(os.getenv("LEGAL_TORRENT_WORKERS", "1") or "1"))
...
def healthcheck():
    ...
    legal_torrent_ready, legal_torrent_reason = is_legal_torrent_pipeline_ready()
    ...
    return {
        ...
        "legal_torrent_autodownload": {
            "enabled": LEGAL_TORRENT_AUTODOWNLOAD_ENABLED,
            "ready": legal_torrent_ready,
            "reason": legal_torrent_reason,
        },
    }
...
def init_postgres_db():
    ...
    cursor.execute(
        """
        CREATE TABLE IF NOT EXISTS legal_torrent_sources (
            id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
            media_type TEXT NOT NULL DEFAULT 'movie',
            movie_id INTEGER NOT NULL,
            title TEXT NOT NULL,
            ...
            CONSTRAINT legal_torrent_sources_media_key UNIQUE (media_type, movie_id)
        )
        """
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_legal_torrent_sources_active ON legal_torrent_sources(media_type, movie_id, is_active)"
    )
    cursor.execute(
        """
        CREATE TABLE IF NOT EXISTS legal_torrent_download_jobs (
            id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
            user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
            source_id INTEGER REFERENCES legal_torrent_sources(id) ON DELETE SET NULL,
            media_type TEXT NOT NULL DEFAULT 'movie',
            movie_id INTEGER NOT NULL,
            title TEXT NOT NULL,
            torrent_url TEXT NOT NULL,
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            CONSTRAINT legal_torrent_download_jobs_user_media_key UNIQUE (user_id, media_type, movie_id)
        )
        """
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_legal_torrent_download_jobs_user_status ON legal_torrent_download_jobs(user_id, status, created_at DESC)"
    )
...
def init_sqlite_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS legal_torrent_sources (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        media_type TEXT NOT NULL DEFAULT 'movie',
                        movie_id INTEGER NOT NULL,
                        title TEXT NOT NULL,
                        ...
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        UNIQUE(media_type, movie_id))''')
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_legal_torrent_sources_active ON legal_torrent_sources(media_type, movie_id, is_active)"
    )
    cursor.execute('''CREATE TABLE IF NOT EXISTS legal_torrent_download_jobs (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        user_id INTEGER,
                        source_id INTEGER,
                        media_type TEXT NOT NULL DEFAULT 'movie',
...
def normalize_legal_torrent_url(value: str) -> str:
    normalized = str(value or "").strip()
    parsed = urlparse(normalized)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise HTTPException(status_code=400, detail="La source torrent doit être une URL HTTPS/HTTP directe.")

    hostname = (parsed.hostname or "").lower()
    if not LEGAL_TORRENT_ALLOWED_HOSTS or hostname not in LEGAL_TORRENT_ALLOWED_HOSTS:
        raise HTTPException(status_code=400, detail="Hôte torrent non autorisé pour les sources légales.")

    if not parsed.path.lower().endswith(".torrent"):
        raise HTTPException(status_code=400, detail="Seules les URL .torrent directes sont acceptées.")

    return normalized


def sanitize_drive_path_segment(value: str) -> str:
    cleaned = re.sub(r"[\\/:*?\"<>|]+", " ", str(value or "")).strip()
    cleaned = re.sub(r"\s+", " ", cleaned)
    return cleaned[:120] or "Qulte"


def is_legal_torrent_pipeline_ready() -> tuple[bool, str]:
    if not LEGAL_TORRENT_AUTODOWNLOAD_ENABLED:
        return False, "Fonction désactivée."
    if not LEGAL_TORRENT_ALLOWED_HOSTS:
        return False, "Aucun hôte légal autorisé."
    if not LEGAL_TORRENT_RCLONE_DESTINATION:
        return False, "Destination Google Drive/rclone manquante."
    if shutil.which("aria2c") is None:
        return False, "aria2c n'est pas installé sur le serveur."
    if shutil.which("rclone") is None:
        return False, "rclone n'est pas installé ou configuré sur le serveur."
    return True, "ok"


def serialize_legal_torrent_source_row(row: Any) -> dict:
    return {
        "id": int(row_get_value(row, "id", 0)),
        "media_type": str(row_get_value(row, "media_type", 1) or "movie"),
        "movie_id": int(row_get_value(row, "movie_id", 2)),
        "title": str(row_get_value(row, "title", 3) or ""),
        "torrent_url": str(row_get_value(row, "torrent_url", 4) or ""),
        "source_page_url": str(row_get_value(row, "source_page_url", 5) or ""),
        "license_label": str(row_get_value(row, "license_label", 6) or ""),
        "license_url": str(row_get_value(row, "license_url", 7) or ""),
        "is_active": bool(row_get_value(row, "is_active", 8)),
        "created_at": serialize_db_datetime(row_get_value(row, "created_at", 9)),
        "updated_at": serialize_db_datetime(row_get_value(row, "updated_at", 10)),
    }


def serialize_legal_torrent_job_row(row: Any) -> dict:
    return {
        "id": int(row_get_value(row, "id", 0)),
        "user_id": int(row_get_value(row, "user_id", 1)),
        "media_type": str(row_get_value(row, "media_type", 2) or "movie"),
        "movie_id": int(row_get_value(row, "movie_id", 3)),
        "title": str(row_get_value(row, "title", 4) or ""),
        "status": str(row_get_value(row, "status", 5) or "queued"),
        "error": row_get_value(row, "error", 6),
        "drive_destination": row_get_value(row, "drive_destination", 7),
        "drive_file_url": row_get_value(row, "drive_file_url", 8),
        "created_at": serialize_db_datetime(row_get_value(row, "created_at", 9)),
        "updated_at": serialize_db_datetime(row_get_value(row, "updated_at", 10)),
        "started_at": serialize_db_datetime(row_get_value(row, "started_at", 11)),
        "completed_at": serialize_db_datetime(row_get_value(row, "completed_at", 12)),
    }


def find_legal_torrent_source(cursor, media_type: str, media_id: int):
    cursor.execute(
        """
        SELECT id, media_type, movie_id, title, torrent_url, source_page_url, license_label, license_url, is_active, created_at, updated_at
        FROM legal_torrent_sources
        WHERE media_type = {param} AND movie_id = {param} AND is_active = 1
        """.format(param=SQL_PARAM),
        (media_type, int(media_id)),
    )
    return cursor.fetchone()


def update_legal_torrent_job_status(
    job_id: int,
    status_value: str,
    *,
    error: Optional[str] = None,
    local_path: Optional[str] = None,
    drive_destination: Optional[str] = None,
    ...
    try:
        ...
        cursor.execute(
            """
            UPDATE legal_torrent_download_jobs
            SET status = {param},
                error = {param},
                local_path = COALESCE({param}, local_path),
                drive_destination = COALESCE({param}, drive_destination),
                started_at = CASE WHEN started_at IS NULL AND {param} IN ('downloading', 'uploading') THEN CURRENT_TIMESTAMP ELSE started_at END,
                completed_at = CASE WHEN {param} = 1 THEN CURRENT_TIMESTAMP ELSE completed_at END,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = {param}
            """.format(param=SQL_PARAM),
            (
...
def run_legal_torrent_download_job(job_id: int):
    ready, reason = is_legal_torrent_pipeline_ready()
    if not ready:
        update_legal_torrent_job_status(job_id, "failed", error=reason, completed=True)
        return
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT id, user_id, source_id, media_type, movie_id, title, torrent_url
            FROM legal_torrent_download_jobs
            WHERE id = {param}
            """.format(param=SQL_PARAM),
            (int(job_id),),
        )
    ...
    title = str(row_get_value(row, "title", 5) or f"media-{row_get_value(row, 'movie_id', 4)}")
    torrent_url = str(row_get_value(row, "torrent_url", 6) or "")
    try:
        normalize_legal_torrent_url(torrent_url)
    except HTTPException as exc:
        update_legal_torrent_job_status(job_id, "failed", error=str(exc.detail), completed=True)
        return

    job_dir = os.path.join(LEGAL_TORRENT_DOWNLOAD_DIR, str(int(job_id)))
    os.makedirs(job_dir, exist_ok=True)
    update_legal_torrent_job_status(job_id, "downloading", local_path=job_dir)
    ...
    try:
        subprocess.run(
            aria2_command,
            check=True,
            timeout=LEGAL_TORRENT_JOB_TIMEOUT_SECONDS,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        )
    except subprocess.TimeoutExpired:
        update_legal_torrent_job_status(job_id, "failed", error="Téléchargement trop long.", completed=True)
        return
    except subprocess.CalledProcessError as exc:
        stderr = (exc.stderr or "").strip()[-500:]
        update_legal_torrent_job_status(job_id, "failed", error=stderr or "Échec du téléchargement torrent.", completed=True)
        return

    if not any(
        os.path.isfile(os.path.join(root, file_name)) and not file_name.endswith(".aria2")
        for root, _, files in os.walk(job_dir)
        for file_name in files
    ):
        update_legal_torrent_job_status(job_id, "failed", error="Aucun fichier téléchargé.", completed=True)
        return

    folder_name = f"{sanitize_drive_path_segment(title)} - TMDB {int(row_get_value(row, 'movie_id', 4))}"
    drive_destination = f"{LEGAL_TORRENT_RCLONE_DESTINATION.rstrip('/')}/{folder_name}"
    update_legal_torrent_job_status(job_id, "uploading", drive_destination=drive_destination)

    try:
        subprocess.run(
            ["rclone", "copy", job_dir, drive_destination],
            check=True,
            timeout=LEGAL_TORRENT_JOB_TIMEOUT_SECONDS,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        update_legal_torrent_job_status(job_id, "failed", error="Upload Google Drive trop long.", completed=True)
        return
    except subprocess.CalledProcessError as exc:
        stderr = (exc.stderr or "").strip()[-500:]
        update_legal_torrent_job_status(job_id, "failed", error=stderr or "Échec de l'upload Google Drive.", completed=True)
        return

    update_legal_torrent_job_status(job_id, "completed", drive_destination=drive_destination, completed=True)


def submit_legal_torrent_job(job_id: int):
    legal_torrent_executor.submit(run_legal_torrent_download_job, int(job_id))


def enqueue_legal_torrent_download_if_available(user_id: int, media_type: str, media_id: int) -> Optional[int]:
    if not LEGAL_TORRENT_AUTODOWNLOAD_ENABLED:
        return None
    ...
    try:
        cursor = conn.cursor()
        source = find_legal_torrent_source(cursor, media_type, media_id)
        if not source:
            return None

        source_id = int(row_get_value(source, "id", 0))
        title = str(row_get_value(source, "title", 3) or f"TMDB {media_id}")
        torrent_url = normalize_legal_torrent_url(str(row_get_value(source, "torrent_url", 4) or ""))
        ...
        if existing_job:
            existing_status = str(row_get_value(existing_job, "status", 1) or "")
            if existing_status in {"queued", "downloading", "uploading", "completed"}:
                return None
            job_id = int(row_get_value(existing_job, "id", 0))
            cursor.execute(
                """
                UPDATE legal_torrent_download_jobs
                SET source_id = {param},
                ...
                WHERE id = {param}
                """.format(param=SQL_PARAM),
                (source_id, title, torrent_url, job_id),
            )
        else:
            job_id = execute_insert_and_get_id(
                cursor,
                """
                INSERT INTO legal_torrent_download_jobs (
                    user_id, source_id, media_type, movie_id, title, torrent_url, status
                ) VALUES (
                    {param}, {param}, {param}, {param}, {param}, {param}, 'queued'
                )
                """.format(param=SQL_PARAM),
                (int(user_id), source_id, media_type, int(media_id), title, torrent_url),
            )
        conn.commit()
    except Exception:
        conn.rollback()
        logger.exception("Impossible de créer le job de torrent légal.")
        return None
    finally:
        conn.close()

    if job_id is not None:
        submit_legal_torrent_job(job_id)
    return job_id
...
def purge_user_data(cursor, user_id: int, *, delete_account: bool) -> tuple[dict[str, int], Optional[str]]:
    ...
    cursor.execute(f"DELETE FROM legal_torrent_download_jobs WHERE user_id = {SQL_PARAM}", (user_id,))
    reset_counts["legal_torrent_download_jobs"] = max(cursor.rowcount, 0)
...
class LegalTorrentSourcePayload(BaseModel):
    media_type: str = "movie"
    movie_id: int
    title: str
    torrent_url: str
...
def require_legal_torrent_admin(x_qulte_admin_token: Optional[str] = Header(default=None)):
    if not LEGAL_TORRENT_ADMIN_TOKEN or x_qulte_admin_token != LEGAL_TORRENT_ADMIN_TOKEN:
...
@app.post("/admin/legal-torrent-sources")
def upsert_legal_torrent_source(
    payload: LegalTorrentSourcePayload,
    _: bool = Depends(require_legal_torrent_admin),
):
    media_type = normalize_media_type(payload.media_type)
    title = payload.title.strip()
    license_label = payload.license_label.strip()
    if not title:
        raise HTTPException(status_code=400, detail="Titre obligatoire.")
    if not license_label:
        raise HTTPException(status_code=400, detail="Licence obligatoire.")
    torrent_url = normalize_legal_torrent_url(payload.torrent_url)
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO legal_torrent_sources (
                media_type, movie_id, title, torrent_url, source_page_url, license_label, license_url, is_active, updated_at
            ) VALUES (
                {param}, {param}, {param}, {param}, {param}, {param}, {param}, {param}, CURRENT_TIMESTAMP
            )
            ON CONFLICT(media_type, movie_id) DO UPDATE SET
            ...
            (
                media_type,
                int(payload.movie_id),
                title,
                torrent_url,
                payload.source_page_url.strip(),
                license_label,
        ...
        cursor.execute(
            ...
            SELECT id, media_type, movie_id, title, torrent_url, source_page_url, license_label, license_url, is_active, created_at, updated_at
            FROM legal_torrent_sources
            WHERE media_type = {param} AND movie_id = {param}
            """.format(param=SQL_PARAM),
            (media_type, int(payload.movie_id)),
        )
        row = cursor.fetchone()
        conn.commit()
    finally:
    ...
    return serialize_legal_torrent_source_row(row)


@app.get("/admin/legal-torrent-sources")
def list_legal_torrent_sources(_: bool = Depends(require_legal_torrent_admin)):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
    ...
    return {"items": [serialize_legal_torrent_source_row(row) for row in rows]}


@app.get("/legal-torrents/jobs")
def list_my_legal_torrent_jobs(current_user: dict = Depends(get_current_user)):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
            """
            SELECT id, user_id, media_type, movie_id, title, status, error, drive_destination, drive_file_url, created_at, updated_at, started_at, completed_at
            FROM legal_torrent_download_jobs
            WHERE user_id = {param}
            ORDER BY created_at DESC
            LIMIT 50
    ...
    return {
        ...
        "ready": ready,
        "reason": reason,
        "items": [serialize_legal_torrent_job_row(row) for row in rows],
    }


@app.get("/legal-torrents/status/{media_type}/{media_id}")
def get_legal_torrent_status(
    media_type: str,
    media_id: int,
    current_user: dict = Depends(get_current_user),
):
    ...
    try:
        cursor = conn.cursor()
        source = find_legal_torrent_source(cursor, normalized_media_type, media_id)
        cursor.execute(
            """
            SELECT id, user_id, media_type, movie_id, title, status, error, drive_destination, drive_file_url, created_at, updated_at, started_at, completed_at
            FROM legal_torrent_download_jobs
            WHERE user_id = {param} AND media_type = {param} AND movie_id = {param}
            """.format(param=SQL_PARAM),
            (current_user["id"], normalized_media_type, int(media_id)),
        )
        job = cursor.fetchone()
    finally:
        conn.close()
    ...
    return {
        ...
        "ready": ready,
        "reason": reason,
        "has_legal_source": source is not None,
        "source": serialize_legal_torrent_source_row(source) if source else None,
        "job": serialize_legal_torrent_job_row(job) if job else None,
    }
...
def add_media_to_specific_playlist(
    ...
    should_enqueue_legal_download = False
    ...
    if info:
        try:
            ...
            should_enqueue_legal_download = playlist_id == WATCH_LATER_SYSTEM_ID
            conn.commit()
        except DBIntegrityError:
            conn.rollback()

    conn.close()
    if should_enqueue_legal_download:
        enqueue_legal_torrent_download_if_available(current_user["id"], normalized_media_type, int(media_id))
    return {"status": "added"}
