...
function NativeTipJarProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <TipJarContext.Provider value={contextValue}>
      {children}
      <Modal visible={visible} transparent animationType="fade" onRequestClose={closeTipJar}>
        <View style={styles.overlay}>
          <Pressable style={styles.backdrop} onPress={closeTipJar} />
          <View
            style={[
              styles.card,
              {
          ...
          >
            <View style={styles.header}>
              ...
              <Pressable onPress={closeTipJar} hitSlop={10} disabled={Boolean(purchasingProductId)}>
...
export function TipJarProvider({ children }: { children: ReactNode }) {
  const fallbackContextValue = useMemo<TipJarContextValue>(
    () => ({
      openTipJar: () => {
  ...
  if (Constants.executionEnvironment === 'storeClient') {
    return <TipJarContext.Provider value={fallbackContextValue}>{children}</TipJarContext.Provider>;
  }

  return <NativeTipJarProvider>{children}</NativeTipJarProvider>;
}
