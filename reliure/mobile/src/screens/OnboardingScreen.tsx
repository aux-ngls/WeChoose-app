...
const APP_PRINCIPLE_STEPS: Array<{
  ...
  },
  {
    ...
    text: 'Reliure regarde les thèmes, les auteurs, les genres et les notes pour proposer mieux.',
  },
  {
    icon: 'people-outline',
  ...
  return (
    <AppScreen>
      ...
      <Pressable onPress={() => void handleSubmit()} style={[styles.primaryButton, { backgroundColor: canContinue ? theme.colors.accent : theme.rgba.cardStrong }]} disabled={!canContinue}>
        {saving ? <ActivityIndicator color={theme.colors.accentText} /> : <Text style={[styles.primaryButtonLabel, { color: canContinue ? theme.colors.accentText : theme.colors.textMuted }]}>{canContinue ? 'Lancer Reliure' : `Encore ${missingMoviesCount} signal${missingMoviesCount > 1 ? 's' : ''}`}</Text>}
      </Pressable>
    </AppScreen>
  );
}
