...
def get_db_connection(*, row_factory: bool = False):
    db_path = os.getenv("RELIURE_DB_PATH", os.path.join(BASE_DIR, "reliure.db"))
...
try:
    ...
    movies_df = pickle.load(open(data_path, "rb"))
...
# --- 5. OUTILS CATALOGUE (compatibles avec les helpers TMDB de Qulte) ---
def get_catalog_row(movie_id: int) -> Optional[dict]:
    movie_index = movie_index_by_id.get(int(movie_id))
    if movie_index is None or movies_df.empty:
        return None
    return dict(movies_df.iloc[movie_index])
...
def get_tmdb_details(movie_id):
    ...
    if catalog_row:
        genres = [
            item.get("name")
            for item in ast.literal_eval(str(catalog_row.get("genres") or "[]"))
            if isinstance(item, dict) and item.get("name")
        ]
        return {
            "id": int(catalog_row["id"]),
            ...
            "runtime": int(catalog_row.get("runtime") or 0),
            "tagline": str(catalog_row.get("tagline") or ""),
            "genres": genres[:4],
...
def download_mobile_archive():
    ...
    return FileResponse(
        ...
        filename="reliure-mobile.tar.gz",
    )
...
<html lang="fr">
  <head>
    ...
    <title>Reliure Preview Player</title>
    <style>
      html, body {{
        margin: 0;
        width: 100%;
